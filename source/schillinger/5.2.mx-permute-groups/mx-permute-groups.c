...
void mx_permute_groups_assist(t_mx_permute_groups *x, void *b, long m, long a,
                              char *s);
void mx_permute_groups_group(t_mx_permute_groups *x, t_symbol *s, long argc,
                             t_atom *argv);
...
void findPermutations(t_symbol **arr, t_atom_long index, t_atom_long n,
                      t_symbol ***result, int *r_index);
...
void ext_main(void *r)
{
    ...
    c = class_new("mx-permute-groups", (method)mx_permute_groups_new,
                  (method)mx_permute_groups_free, sizeof(t_mx_permute_groups),
                  NULL, A_GIMME, 0);
    ...
    class_addmethod(c, (method)mx_permute_groups_circular, "circular", A_LONG,
                    0);
    class_addmethod(c, (method)mx_permute_groups_anticircular, "anticircular",
                    A_LONG, 0);
...
{
    // allocate class
    t_mx_permute_groups *x =
        (t_mx_permute_groups *)object_alloc(mx_permute_groups_class);
                             ...
                             t_atom *argv)
{
    ...
    x->groups = (t_atom_long **)sysmem_newptrclear(x->group_amt *
                                                   sizeof(t_atom_long *));
    ...
    for (int i = 1; i <= argc; i++) {
        if (atom_gettype(argv + i) == 3 || i == argc) {
            x->groups[g_c] =
                (t_atom_long *)sysmem_newptrclear(ing_c * sizeof(t_atom_long));
    ...
    x->sym_groups =
        (t_symbol **)sysmem_newptr(x->group_amt * sizeof(t_symbol *));
    ...
    x->permutations =
        (t_symbol ***)sysmem_newptrclear(x->unq_perm * sizeof(t_symbol **));

    for (int i = 0; i < x->unq_perm; i++) {
        // x->permutations[i] = (t_symbol
        // **)sysmem_newptrclear(argc*sizeof(t_symbol *));
        x->permutations[i] =
            (t_symbol **)sysmem_newptrclear(x->group_amt * sizeof(t_symbol *));
    }
...
void print(t_mx_permute_groups *x, t_symbol **s)
{
    ...
    long_array = (t_atom_long **)sysmem_newptrclear(x->group_amt *
                                                    sizeof(t_atom_long *));
    ...
    for (int i = 0; i < x->group_amt; i++) {
        ...
        long_array[i] =
            (t_atom_long *)sysmem_newptrclear(c * sizeof(t_atom_long));
    ...
    t_atom_long *patbin =
        (t_atom_long *)sysmem_newptrclear(pat_sum * sizeof(t_atom_long));
