...
void mx_autogroup_patbin(t_mx_autogroup *x, t_symbol *s, long argc,
                         t_atom *argv);
...
void ext_main(void *r)
{
    ...
    c = class_new("mx-autogroup", (method)mx_autogroup_new,
                  (method)mx_autogroup_free, sizeof(t_mx_autogroup), NULL,
                  A_GIMME, 0);
...
void mx_autogroup_free(t_mx_autogroup *x)
{
    if (x->pattern) sysmem_freeptr(x->pattern);
...
void mx_autogroup_assist(t_mx_autogroup *x, void *b, long m, long a, char *s)
{
    if (m == ASSIST_INLET) {
        ...
        switch (a) {
            ...
            case 2:
                sprintf(s,
                        "(message) Connect to umenu to see possible groupings");
                break;
        }
    }
}
...
void mx_autogroup_pat(t_mx_autogroup *x, t_symbol *s, long argc, t_atom *argv)
{
    ...
    x->pattern =
        (t_atom_long *)sysmem_newptrclear(x->pat_len * sizeof(t_atom_long));
...
void mx_autogroup_bang(t_mx_autogroup *x)
{
    if (!x->pattern) {
        ...
        outlet_anything(x->group_out, gensym("group"), x->group_size,
                        x->output);
    }
}
